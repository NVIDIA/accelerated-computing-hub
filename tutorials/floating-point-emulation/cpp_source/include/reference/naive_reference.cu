...
namespace example {
    ...
    template<class AccumType, class AEngine, class ALayout, class BEngine, class BLayout>
    CUTE_HOST_DEVICE void dot(AccumType&                            accum,
    ...
    template<class AEngine,
             ...
             class Alpha,
             class Beta>
    __global__ void reference_gemm_naive_kernel(Alpha                                alpha,
                                                cute::Tensor<AEngine, ALayout> const tensor_a,
                                                cute::Tensor<BEngine, BLayout> const tensor_b,
                                                Beta                                 beta,
    ...
    void reference_gemm_naive_device(unsigned_tuple const&           gemm_shape,
        ...
        auto make_tensor = [](auto ptr, auto sx, auto sy, auto ld, bool col_major) {
            return cute::make_tensor(
                ptr,
                cute::make_layout(cute::make_shape(sx, sy), cute::make_stride(col_major ? 1 : ld, col_major ? ld : 1)));
        };
...
#define REFERENCE_FOR_TYPE(Prec)                                                           \
    template void reference_gemm_naive_device<Prec>(unsigned_tuple const&      gemm_shape, \
                                                    arr_tuple const&           gemm_arr,   \
                                                    unsigned_tuple const&      gemm_ld,    \
                                                    Prec const&                alpha,      \
