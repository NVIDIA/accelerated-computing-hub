...
namespace example {
    ...
    struct cublaslt_runner {
        ...
        unsigned                   num_a_elements {};

        cublasLtMatrixLayout_t b_desc {};
        unsigned               num_b_elements {};

        cublasLtMatrixLayout_t c_desc {};
        unsigned               num_c_elements {};
        ...
        mutable thrust::device_vector<char> workspace_vector = thrust::device_vector<char>(workspace_size_in_bytes);
        ...
        void initialize(GEMMShape gemm_shape, GEMMArr gemm_arr, GEMMLD gemm_ld) {
            ...
            CUBLAS_CHECK_AND_EXIT(cublasLtMatrixLayoutSetAttribute(
                a_desc, CUBLASLT_MATRIX_LAYOUT_ORDER, &cublas_order_a, sizeof(cublas_order_a)));
            num_a_elements = arr_a == cublasdx::col_major ? k * lda : m * lda ;
            ...
            num_b_elements = arr_b == cublasdx::col_major ? n * ldb : k * ldb;

            CUBLAS_CHECK_AND_EXIT(cublasLtMatrixLayoutCreate(&c_desc, cublas_data_type_c, m, n, ldc));
            CUBLAS_CHECK_AND_EXIT(cublasLtMatrixLayoutSetAttribute(
                c_desc, CUBLASLT_MATRIX_LAYOUT_ORDER, &cublas_order_c, sizeof(cublas_order_c)));
            num_c_elements = arr_c == cublasdx::col_major ? n * ldc : m * ldc;
            ...
            CUBLAS_CHECK_AND_EXIT(cublasLtMatmulPreferenceSetAttribute(preference,
                                                                       CUBLASLT_MATMUL_PREF_MAX_WORKSPACE_BYTES,
                                                                       &workspace_size_in_bytes,
                                                                       sizeof(workspace_size_in_bytes)));

            CUBLAS_CHECK_AND_EXIT(cublasLtMatmulAlgoGetHeuristic(lt_handle,
                                                                 operation_desc,
                                                                 a_desc,
                                                                 b_desc,
                                                                 c_desc,
            ...
            auto runner = [&](cudaStream_t stream) {
                CUBLAS_CHECK_AND_EXIT(
                    cublasLtMatmul(lt_handle,
                                   ...
                                   reinterpret_cast<void*>(thrust::raw_pointer_cast(workspace_vector.data())),
                                   workspace_size_in_bytes,
                                   stream));
            };
        ...
        [[nodiscard]] std::vector<CComputeType> execute_with_results(CComputeType const& alpha,
                                                                     AComputeType const* a,
                                                                     BComputeType const* b,
                                                                     CComputeType const& beta,
                                                                     CComputeType*       c,
            ...
            CUDA_CHECK_AND_EXIT(
                cudaMemcpy(results.data(), c, results.size() * sizeof(CComputeType), cudaMemcpyDeviceToHost));
        ...
        [[nodiscard]] float execute_with_time(CComputeType const& alpha,
            ...
            // Cast away const, but data will only be reloaded for L2 reset
            // no changes will actually happen to it
            auto mutable_a = const_cast<AComputeType*>(a);
            auto mutable_b = const_cast<BComputeType*>(b);

            // Prepare host data for L2 reset
            std::vector<AComputeType> host_a_data(num_a_elements);
            std::vector<BComputeType> host_b_data(num_b_elements);
            std::vector<CComputeType> host_c_data(num_c_elements);

            CUDA_CHECK_AND_EXIT(cudaMemcpy((void*)host_a_data.data(),
                                           (void const*)a,
                                           host_a_data.size() * sizeof(AComputeType),
                                           cudaMemcpyDeviceToHost));
            CUDA_CHECK_AND_EXIT(cudaMemcpy((void*)host_b_data.data(),
                                           (void const*)b,
                                           host_b_data.size() * sizeof(BComputeType),
                                           cudaMemcpyDeviceToHost));
            CUDA_CHECK_AND_EXIT(cudaMemcpy((void*)host_c_data.data(),
                                           (void*)c,
                                           host_c_data.size() * sizeof(CComputeType),
                                           cudaMemcpyDeviceToHost));

            for (int algo_idx = 0; algo_idx < returned_results; ++algo_idx) {
                // Reset L2 cache
                {
                    CUDA_CHECK_AND_EXIT(cudaMemcpy((void*)mutable_a,
                                                   (void*)host_a_data.data(),
                                                   host_a_data.size() * sizeof(AComputeType),
                                                   cudaMemcpyHostToDevice));
                    CUDA_CHECK_AND_EXIT(cudaMemcpy((void*)mutable_b,
                                                   (void*)host_b_data.data(),
                                                   host_b_data.size() * sizeof(BComputeType),
                                                   cudaMemcpyHostToDevice));
                    CUDA_CHECK_AND_EXIT(cudaMemcpy((void*)c,
                                                   (void*)host_c_data.data(),
                                                   host_c_data.size() * sizeof(CComputeType),
                                                   cudaMemcpyHostToDevice));
                }

                auto time = tutorial::measure::execution(
            ...
            auto time_cublas = tutorial::measure::execution(
        ...
        ~cublaslt_runner() {
            if (preference)
                CUBLAS_CHECK_AND_EXIT(cublasLtMatmulPreferenceDestroy(preference));
            if (c_desc)
                CUBLAS_CHECK_AND_EXIT(cublasLtMatrixLayoutDestroy(c_desc));
            if (b_desc)
                CUBLAS_CHECK_AND_EXIT(cublasLtMatrixLayoutDestroy(b_desc));
            if (a_desc)
                CUBLAS_CHECK_AND_EXIT(cublasLtMatrixLayoutDestroy(a_desc));
            if (operation_desc)
                CUBLAS_CHECK_AND_EXIT(cublasLtMatmulDescDestroy(operation_desc));
        }
    };
} // namespace example
