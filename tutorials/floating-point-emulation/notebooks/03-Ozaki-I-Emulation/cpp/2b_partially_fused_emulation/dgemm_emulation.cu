...
auto run_tutorial_kernel(double         alpha,
    ...
    auto           d_products_storage =
        tutorial::get_empty_device_tensor<accumulator_value_type, arr_c>(shape_c_rows, shape_c_cols, static_slices);
    // Capturing a structured binding into lambda is a C++20 feature
	...
	#include "pipeline_config.hpp.inc"
        ...
        dim3 grid(shape_a_rows / static_tile_m(), shape_b_cols / static_tile_n());
        auto kernel = fused_epilogue_kernel<Slices, BLAS, decltype(device_pipeline), decltype(tensor_products)>;
        ...
        auto run_fused_matmul = [&](auto str) {
            kernel<<<grid, device_pipeline.get_block_dim(), shared_memory_size, str>>>(device_pipeline,
                                                                                        tensor_products);
        };

        auto time_ms = tutorial::measure::execution(run_fused_matmul, warm_up_runs, kernel_runs, stream);
        total_time += time_ms;
