...

    {
        ...
        auto run_fused_epilogue = [&](auto str) {
            kernel<<<grid, device_pipeline.get_block_dim(), shared_memory_size, str>>>(
                device_pipeline, alpha, beta, dummy_tensor_c, tensor_shift_a, tensor_shift_b);
        };
        ...
        kernel<<<grid, device_pipeline.get_block_dim(), shared_memory_size, stream>>>(
            device_pipeline, alpha, beta, tensor_c, tensor_shift_a, tensor_shift_b);
