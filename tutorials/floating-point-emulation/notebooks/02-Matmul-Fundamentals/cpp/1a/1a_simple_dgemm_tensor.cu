...
auto run_tutorial_kernel(Alpha          alpha,
    ...
    int const grid_dim_x = cuda::ceil_div(size_m, block_dim_x);
    int const grid_dim_y = cuda::ceil_div(size_n, block_dim_y);

    auto const block_dim = dim3(block_dim_x, block_dim_y);
    ...
    auto run_kernel = [&](auto& str) {
    	kernel_1a_simple_dgemm<block_dim_x * block_dim_y>
            <<<grid_dim, block_dim, 0, stream>>>(alpha, tensor_a, tensor_b, beta, tensor_c);
    };
...
int main(int argc, char** argv) {
    ...
    for (tutorial::gemm_problem_t problem : problems) {
        ...
        // 1. Generate tensors with random data
