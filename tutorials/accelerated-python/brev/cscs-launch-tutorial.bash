#!/usr/bin/env bash
# Launch the Accelerated Python/PyHPC web services from a Daint login node or
# run their Slurm job.
...
Options:
  ...
  --cpus-per-task COUNT  CPUs for thread/rank scaling (default: 32)
...
is_ach_checkout() {
    ...
    origin_url=${origin_url,,}
    case "${origin_url}" in
        https://github.com/*/accelerated-computing-hub|\
        git@github.com:*/accelerated-computing-hub|\
        ssh://git@github.com/*/accelerated-computing-hub)
            return 0
            ;;
        *)
            return 1
...
prepare_checkout() {
    ...
    if [ ! -e "${repo}" ] && [ ! -L "${repo}" ]; then
    ...
    elif ! checkout_root=$(git -C "${repo}" rev-parse --show-toplevel 2>/dev/null) || \
         [ "$(realpath -m -- "${checkout_root}")" != "${repo}" ]; then
        echo "Error: checkout path is not the root of a working Git checkout: ${repo}" >&2
        return 1
    elif ! is_ach_checkout "${repo}"; then
        echo "Error: checkout origin is not an accelerated-computing-hub repository: ${repo}" >&2
        return 1
    else
        local branch
        ...
        local legacy_entry=
        if [ -d "${repo}/tutorials/pyhpc" ]; then
            legacy_entry=$(find "${repo}/tutorials/pyhpc" -mindepth 1 \
                -print -quit)
        fi
        if [ -n "${legacy_entry}" ]; then
            echo "Found the former standalone PyHPC tree in this checkout."
            echo "Preserving it, including ignored student output, and using a sibling checkout."
            CHECKOUT_NEEDS_FRESH=true
        elif [ -z "${status}" ]; then
            ...
            if git -C "${repo}" show-ref --verify --quiet \
                ...
                if git -C "${repo}" merge-base --is-ancestor \
                    ...
                    echo "The clean checkout cannot fast-forward after the selected branch was rewritten."
                    echo "Preserving it and preparing a fresh checkout for this launch."
                    CHECKOUT_NEEDS_FRESH=true
                fi
            else
...
login_main() {
    ...
    case "${cpus_per_task}" in
        ''|0|*[!0-9]*) echo "Error: --cpus-per-task must be a positive integer." >&2; return 2 ;;
    esac
    if [ -z "${account}" ]; then
    ...
    if [ "${CHECKOUT_NEEDS_FRESH}" = true ] || \
       [ ! -f "${repo}/tutorials/accelerated-python/notebooks/syllabi/pyhpc__cupy_kernels_mpi_jax_omp_interop__2_days.ipynb" ]; then
        ...
        while [ "${sibling_index}" -le 100 ]; do
            repo=${sibling_base}
            if [ "${sibling_index}" -gt 1 ]; then
                repo="${sibling_base}-${sibling_index}"
            fi
            local candidate_existed=false
            if [ -e "${repo}" ] || [ -L "${repo}" ]; then
                candidate_existed=true
                local candidate_root=
                candidate_root=$(git -C "${repo}" rev-parse --show-toplevel 2>/dev/null || true)
                if [ -z "${candidate_root}" ] || \
                   [ "$(realpath -m -- "${candidate_root}")" != "${repo}" ]; then
                    echo "Skipping occupied path that is not a checkout root: ${repo}."
                    sibling_index=$((sibling_index + 1))
                    continue
                fi
                if ! is_ach_checkout "${repo}"; then
                    echo "Skipping unrelated sibling checkout ${repo}."
                    sibling_index=$((sibling_index + 1))
                    continue
                fi
                if [ -z "$(git -C "${repo}" branch --show-current)" ]; then
                    echo "Skipping detached sibling checkout ${repo}."
                    sibling_index=$((sibling_index + 1))
                    continue
                fi
            fi
        ...
        if [ "${sibling_index}" -gt 100 ]; then
            echo "Error: no usable sibling checkout path found after 100 attempts." >&2
            return 1
        fi
    fi
    ...
    if [ ! -f "${prepare_source}" ]; then
        prepare_source="${repo}/brev/prepare-podman-compose.py"
    fi
    ...
    job_id=$(env \
        ACH_REPO="${repo}" \
        ACH_STATE="${state_dir}" \
        ACH_RELEASE_BRANCH="${branch}" \
        ACH_PREPARE_SOURCE="${prepare_source}" \
        sbatch "${sbatch_args[@]}" "${batch_script}" --batch)
    job_id=${job_id%%;*}
    case "${job_id}" in
