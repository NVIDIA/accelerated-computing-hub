...
--account, --state, --partition, --cpus-per-task, --time, and --start-timeout.
CSCS_CPUS_PER_TASK and CSCS_RESERVATION are forwarded as defaults; an explicit
...
bootstrap_streamed_helpers() {
    ...
    local base_url=${ACH_CSCS_HELPER_BASE_URL:-https://raw.githubusercontent.com/NVIDIA/accelerated-computing-hub/${branch}/tutorials/accelerated-python/brev}
...
if [ ! -x "${prepare_script}" ]; then
    prepare_script="${script_dir}/../../../brev/prepare-podman-compose.py"
fi
...
if [ ! -x "${prepare_script}" ]; then
    echo "Error: missing Compose adapter: ${prepare_script}" >&2
    exit 1
fi
...
launch_defaults=()
if [ -n "${CSCS_CPUS_PER_TASK:-}" ]; then
    launch_defaults+=(--cpus-per-task "${CSCS_CPUS_PER_TASK}")
fi
if [ -n "${CSCS_RESERVATION:-}" ]; then
    launch_defaults+=(--reservation "${CSCS_RESERVATION}")
fi
# Defaults precede forwarded CLI options so the Daint-side parser gives an
# explicit option the final say.
launch_args=("${launch_defaults[@]}" "${launch_args[@]}")
