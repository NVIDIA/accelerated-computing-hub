...
void histogram(cuda::std::span<float> temperature,
...
int main() {
  ...
  for (int write_step = 0; write_step < 100; write_step++) {
    ...
    histogram(
        cuda::std::span<float>{thrust::raw_pointer_cast(d_prev.data()),
                               height * width},
        cuda::std::span<int>{thrust::raw_pointer_cast(d_histogram.data()), 10},
        compute_stream);
