...
# Read a tutorial name from its Compose anchor. Generated syllabus Compose
# files live below notebooks/syllabi rather than directly below brev/, so their
# parent directory is not a reliable tutorial name. Fall back to the historic
# directory layout for older Compose files without the anchor.
compose_tutorial_name() {
    local compose_file=${1}
    local tutorial

    tutorial=$(sed -n \
        's/^[[:space:]]*name:[[:space:]]*&tutorial-name[[:space:]]*\([^[:space:]#]*\).*$/\1/p' \
...
development_volume_name() {
    if [ "${1}" = "accelerated-python" ]; then
        printf '%s_accelerated-computing-hub-dev\n' "${1}"
    else
        printf '%s_accelerated-computing-hub\n' "${1}"
    fi
}
...
setup_docker_volume() {
    ...
    local MOUNT=${2:-true}
    local VOLUME_NAME
    VOLUME_NAME=$(development_volume_name "${ACH_TUTORIAL}")

    # Compose files may version their default volume for a non-destructive
    # content migration. Development helpers still select the disposable
    # volume created below.
    export ACH_VOLUME_NAME="${VOLUME_NAME}"
